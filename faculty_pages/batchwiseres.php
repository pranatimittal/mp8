...
<div class = "manageuser">
  ...
  //$n="";
  $idf= $_SESSION['idf'];
    $sql1 = mysqli_query($con,"SELECT s.name,s.rollno,h.score,h.stud_ans from quiz as q INNER JOIN history as h ON q.quizid = h.quizid INNER JOIN login_student as s on s.email=h.email WHERE q.teacher_id='$idf' AND h.quizid='$testid' AND s.program='$p' AND s.branch='$b' AND s.semester='$s' ORDER BY h.score DESC   ") or die('No Entries');
...
echo '<br><br><br><div align="center">
    <button class="btn"><i class="fa fa-download"></i><a style= "color : white ;" href="exportbatch.php?tid='.$testid.'&p='.$p.'&b='.$b.'&s='.$s.'&id='.$idf.'"><u><b> Export To Excel </b></u></a></button>
    </div>';
